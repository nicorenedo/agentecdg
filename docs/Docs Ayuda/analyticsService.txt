...
/**
 * ✅ FUNCIÓN PARA PIVOTEO CONVERSACIONAL CORREGIDA: Cambia gráficos usando lenguaje natural
 */
async function pivotChart(userId, message, currentChartConfig = {}, chartInteractionType = "pivot", options = {}) {
  ...
  console.log(`[Analytics] 🔄 Pivoting chart with message: "${message}"`);
  
  try {
    // ✅ CORREGIDO: Usar el método correcto del api.js
    ...
    console.log(`[Analytics] ✅ Pivot successful`);
    
    return transformBackendChartData(result, 'pivot_result');
  } catch (error) {
    console.error(`[Analytics] ❌ Error pivoting chart:`, error.message);
    throw error;
  }
}
