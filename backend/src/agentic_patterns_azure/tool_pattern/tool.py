...
def validate_arguments(tool_call: dict, tool_signature: dict) -> dict:
    ...
    for arg_name, arg_value in tool_call["arguments"].items():
        if arg_name not in properties:
            raise ValueError(f"Argument '{arg_name}' not defined in tool signature.")

        expected_type = properties[arg_name].get("type")
        if expected_type not in type_mapping:
            raise ValueError(f"Unsupported type '{expected_type}' for argument '{arg_name}'.")

        # Check type and attempt conversion if needed
        if not isinstance(arg_value, type_mapping[expected_type]):
            try:
                tool_call["arguments"][arg_name] = type_mapping[expected_type](arg_value)
            except Exception as e:
                raise ValueError(
                    f"Failed to convert argument '{arg_name}' to type '{expected_type}': {e}"
                )

        # Special handling for booleans
        if expected_type == "bool":
            tool_call["arguments"][arg_name] = (
                True if tool_call["arguments"][arg_name] else False
            )
